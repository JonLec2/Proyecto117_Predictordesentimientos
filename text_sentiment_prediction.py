...
def predict(text):
    predictemotion=""
    predictemotionimgurl=""
    if text!="":
        sentence=[]
        sentence.append(text)
        sequences=tokenizer.texts_to_sequences(sentence)
        padel=pad_sequences(sequences,maxlen=max_length, padding=padding_type, truncating=trunc_type)
        testinpadel=np.array(padel)
        predictlabel=np.argmax(model.predict(testinpadel), axis=1)
